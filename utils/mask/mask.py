...
def interpolate_mask(tensor, mask, mask_inv):
    ...
    tensorlist = torch.split(tensor, 1, dim=1)
    tensorlist_out = []
    for tensor_1c in tensorlist:
        tensorlist_out.append(torch.nn.functional.conv2d(tensor_1c, kernel, stride=1, padding=1))

    filtered_tensor = torch.cat(tensorlist_out, 1)
